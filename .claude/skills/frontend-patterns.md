...
<Tabs defaultTab="overview">
  <TabList>
    ...
    <Tab id="details">상세정보</Tab>
  </TabList>
</Tabs>
```

### 렌더 Props 패턴 (Render Props Pattern)
...
const { data: markets, loading, error, refetch } = useQuery(
  ...
  {
    onSuccess: data => console.log('조회됨:', data.length, '개의 마켓'),
...
export function useMarkets() {
  ...
  if (!context) throw new Error('useMarkets는 MarketProvider 내에서 사용되어야 합니다')
  return context
}
```
...
```typescript
// ✅ 비용이 높은 연산에 useMemo 사용
const sortedMarkets = useMemo(() => {
  return markets.sort((a, b) => b.volume - a.volume)
}, [markets])

// ✅ 자식에게 전달하는 함수에 useCallback 사용
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('에러 바운더리가 에러를 잡았습니다:', error, errorInfo)
  }
...
**기억하세요**: 최신 프론트엔드 패턴은 유지보수 가능하고 고성능인 사용자 인터페이스를 구현할 수 있게 합니다. 프로젝트의 복잡도에 맞는 패턴을 선택하세요.
